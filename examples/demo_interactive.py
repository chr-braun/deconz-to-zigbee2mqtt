#!/usr/bin/env python3
"""
Demo des interaktiven deCONZ zu Zigbee2MQTT Migration Tools
Simuliert die Benutzerinteraktion für Demonstration
"""
...
def simulate_user_input(prompt, default="", value=None):
    """Simuliere Benutzereingabe für Demo."""
...
def generate_api_key(host, port):
    ...
    print("\n[DEMO] Simuliere: Benutzer drückt ENTER...")
    time.sleep(2)
...
def main():
    ...
    host = simulate_user_input(
        "deCONZ Server IP-Adresse", 
        "192.168.178.76",
        "192.168.178.76"
    )
    
    port = simulate_user_input(
        "deCONZ Port", 
        "4530",
        "4530"
    )
    ...
    mqtt_server = simulate_user_input(
        "MQTT Server URL", 
        "mqtt://localhost",
        "mqtt://localhost"
    )
    
    mqtt_topic = simulate_user_input(
        "MQTT Base Topic", 
        "zigbee2mqtt",
        "zigbee2mqtt"
    )
